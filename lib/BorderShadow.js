...
export default class BorderShadow extends Component {
	render = () => {
		...
		const childArray = React.Children.map(children, (child, i) => {
			return child
		});
		return (
			<View style={[{position:"relative",width:width},style]}>
				{(()=>{
					switch (side){
						case "top":
							return [
								<Svg height={lineWidth} key={side} width={width+lineWidth} style={{position:"absolute",top:(inset?0:-lineWidth)}}>
									<Defs>
						...
						case "bottom":
							return [
								...childArray,
								<Svg height={lineWidth} key={side} width={width+lineWidth} style={{position:"absolute",bottom:(inset?-lineWidth:0)}}>
									<Defs>
